...
* Merges the 0.4.6 release line of `main` into `next`. Both branches had
  independently reached 0.4.6, so the merged package is 0.4.7. Everything
  listed under the `(next)` headings below ships for the first time here,
  alongside main's 0.4.6 entries.
...
* `main`'s infrastructure merged into `next`: the window-edge fix, tie
  attributes kept by `dynet()`, `thread_clock = "relative"`, the
  `thought_chains` dataset and reproduction, the plot defaults, and the CRAN
  preparation (NEWS, word list, `Language: en-GB`), on top of `next`'s null
