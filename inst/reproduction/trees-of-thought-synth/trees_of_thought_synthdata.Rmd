...
subtitle: "The study's networkDynamic / tsna analysis, re-run on the bundled `synthdata`"
output:
  html_document:
    toc: true
    toc_float: true
...
```{r import}
dn <- dynet(synthdata, directed = TRUE, loops = TRUE,
            weight = "weight")
dn
summary(dn)
```
...
heavy <- induce_subgraph(dn, ties = weight > 100)
summary(heavy)
```
...
heavy_structure <- metrics(heavy,
                           measure = c("density", "edges", "reciprocity", "connectedness"),
                           step = 0.1, window = 0.1)
plot(heavy_structure, type = "ridge")
```
...
```{r audit-unchanged}
identical(dn, dynet(synthdata, directed = TRUE, loops = TRUE,
                    weight = "weight"))
```
