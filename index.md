...
To build a temporal network from an interval log, we call
...
`sender` and `receiver`, `source` and `target`, `onset` and `terminus`,
and several other spellings are accepted as well, and times may be
numeric, dates or date-times. Printing the network describes its format
and shows the first spells.
...
To see the network’s dimensions in one table, we call
[`summary()`](https://rdrr.io/r/base/summary.html). The mean snapshot
density is the average over one-unit bins of the share of pairs that are
connected in the bin.
...
Each horizontal segment is one spell.
...
To obtain a graph-level measure in each window, we call
...
To summarise a series, we call
[`summary()`](https://rdrr.io/r/base/summary.html) on it. The table
reports one row per measure with its mean, spread, range and the time at
which it peaked.
...
To draw the series, we call
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) on it.
...
![](reference/figures/README-metrics-plot-1.png)
...
with the network and `measure` for the index. The result has one row per
vertex per time point.
...
To reduce the series to one row per vertex, we call
[`summary()`](https://rdrr.io/r/base/summary.html) on it. The
...
other, we call
[`paths()`](https://pak.dynasite.org/Dynet/reference/paths.md) with the
network and `from` for the source. The result has one row per vertex.
`arrival_time` is the earliest moment the vertex can be reached,
`latency` is that moment minus the time the source started, `n_hops` is
the number of ties the earliest route uses, and `n_paths` is the number
of distinct routes that arrive equally early.
...
To list the routes themselves, we call
[`pathways()`](https://pak.dynasite.org/Dynet/reference/pathways.md)
with the network and `from` for the source. Each row is one distinct
sequence of vertices, with the number of earliest routes that follow it.
...
To draw the routes as a tree, we call
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) on the paths
result. A vertex appears more than once when it is entered at different
times, because each entry time leaves a different set of onward contacts
available.
...
![](reference/figures/README-paths-plot-1.png)
...
with the network; `direction` selects forward, backward or both.
...
the network. The default measures are formation and dissolution.
...
To measure how long ties last, we call
[`durations()`](https://pak.dynasite.org/Dynet/reference/durations.md)
with the network and `measure` for the statistic. With
`measure = "mean"` the result is the mean spell length of each pair;
`unit` moves the question from pairs to individual spells or to vertex
activity.
...
with the network. Burstiness compares the dispersion of the gaps between
a vertex’s events with the exponential reference: 1 is the bursty limit,
...
To count ties within and between groups of vertices, we call
[`mixing()`](https://pak.dynasite.org/Dynet/reference/mixing.md) with
the network and `attribute` for the vertex attribute that defines the
groups. The forum network carries a `role` attribute from
...
with the network and `weight` for the aggregation rule. The result is a
`cograph` network and can be drawn and measured as one.
...
with the network and a condition on the vertex table in `nodes`. A
centrality named in the condition is computed over the whole observed
period.
...
To list the ties active in one window, we call
[`snapshots()`](https://pak.dynasite.org/Dynet/reference/snapshots.md)
with the network and `at` for the time.
...
function applies to it directly.
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) adds the views
that concern time. To draw the network as it stands in one window, we
set `type` to `"network"` and give the time in `at`. To draw a sequence
of windows as small multiples with a shared layout, we set `type` to
`"snapshots"`. To draw the proximity timeline, in which vertices that
interact are placed near each other and their positions are followed
through time, we set `type` to `"proximity"`. The proximity timeline is
described in the article on drawing.

To animate the measurement windows, we call
[`animate()`](https://pak.dynasite.org/Dynet/reference/animate.md) with
the same four grid arguments as the measuring functions and a file name
whose extension selects the encoder; `.gif` needs the `gifski` package
and `.mp4` or `.webm` the `av` package. The article on animating shows
the result.

## Documentation

- [`vignette("dynet")`](https://pak.dynasite.org/Dynet/articles/dynet.md)
  walks through one analysis from log to result.
- [`vignette("building-networks")`](https://pak.dynasite.org/Dynet/articles/building-networks.md)
  covers the four log shapes, vertex attributes, observation windows,
  sessions and censoring.
- [`vignette("ch17-temporal-networks")`](https://pak.dynasite.org/Dynet/articles/ch17-temporal-networks.md)
  re-runs the temporal network chapter of *Learning Analytics Methods
  and Tutorials* on the bundled MOOC forum.
