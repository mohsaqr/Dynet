...
test_that("several directions come back as one stacked measure column", {
  ...
  got <- as.data.frame(centrality_series(dn, measure = "degree",
                                         mode = c("all", "in", "out"),
                                         window = "all"))
...
test_that("the stacked directions agree with one call each", {
  dn <- dynet(school_contacts)
  stacked <- as.data.frame(centrality_series(dn, measure = "degree",
                                             mode = c("in", "out"),
                                             window = "all"))
  alone <- as.data.frame(centrality_series(dn, measure = "degree", mode = "in",
                                           window = "all"))
...
test_that("a single direction keeps the plain measure name it always had", {
  dn <- dynet(school_contacts)
  expect_identical(
    unique(as.data.frame(centrality_series(dn, measure = "degree", mode = "in",
                                           window = "all"))$measure), "degree")
  expect_identical(
    unique(as.data.frame(centrality_series(dn, measure = "degree",
                                           window = "all"))$measure), "degree")
})

test_that("a direction-blind measure is not computed once per direction", {
  dn <- dynet(school_contacts)
  got <- as.data.frame(centrality_series(dn, measure = "betweenness",
...
test_that("mixed measures expand only where direction means something", {
  dn <- dynet(school_contacts)
  got <- as.data.frame(centrality_series(dn, measure = c("degree", "betweenness"),
...
test_that("an unknown direction is refused", {
  dn <- dynet(school_contacts)
  expect_error(centrality_series(dn, measure = "degree", mode = c("in", "sideways")),
               class = "dynet_bad_input")
})
